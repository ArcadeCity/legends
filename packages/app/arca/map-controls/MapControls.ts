...
import * as geoUtils from 'app/arca/geoutils'
import { CameraUtils, EventDispatcher, MapView, MapViewEventNames, MapViewUtils } from 'app/arca/mapview'
...
export class MapControls extends EventDispatcher {
  ...
  private m_recentPanDistancesOrAngles: [number, number, number, number, number] = [0, 0, 0, 0, 0]
  ...
  private handlePan() {
    ...
    if (applyInertia) {
      ...
      if (this.mapView.projection.type === geoUtils.ProjectionType.Planar) {
        panDistanceOrAngle = this.m_lastPanVector.copy(this.m_panDistanceFrameDelta).length()
      } else if (this.mapView.projection.type === geoUtils.ProjectionType.Spherical) {
  ...
  private convertTouchPoint(touch: Touch, oldTouchState?: TouchState): TouchState | undefined {
    ...
    if (oldTouchState !== undefined) {
      ...
      if (toWorld.length() === 0 && !(this.m_touchState.touches.length === 3 && this.tiltEnabled)) {
  ...
  private getWorldPositionWithElevation(
    ...
    if (elevationProviderResult === undefined) {
      ...
      toWorld = MapViewUtils.rayCastWorldCoordinates(this.mapView, to.x, to.y, fromGeoAltitude)
    }
