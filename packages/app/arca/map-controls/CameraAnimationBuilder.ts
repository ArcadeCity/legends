...
import { GeoCoordinates, MathUtils } from 'app/arca/geoutils'
import { LookAtParams, MapView } from 'app/arca/mapview'
...
export class CameraAnimationBuilder {
  ...
  static appendControlPoint(
    ...
    if (
      ...
      (point.timestamp === undefined ||
        point.timestamp <= options.controlPoints[options.controlPoints.length - 1].timestamp)
    ) {
      point.timestamp =
  ...
  static createBowFlyToOptions(
    ...
    let maxAltitude =
      altitude ??
      2 * startWorldTarget.distanceTo(mapView.projection.projectPoint(targetControlPoint.target))
    ...
    const midPoint0 = new ControlPoint({
      ...
      tilt: MathUtils.interpolateAnglesDeg(startControlPoint.tilt, targetControlPoint.tilt, 0.25),
    ...
    const midPoint1 = new ControlPoint({
      ...
      tilt: MathUtils.interpolateAnglesDeg(startControlPoint.tilt, targetControlPoint.tilt, 0.75),
      heading: MathUtils.interpolateAnglesDeg(
        startControlPoint.heading,
        targetControlPoint.heading,
        0.75
