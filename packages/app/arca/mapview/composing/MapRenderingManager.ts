...
import { IBloomEffect, IOutlineEffect, ISepiaEffect, IVignetteEffect } from 'app/arca/datasource-protocol'
import { SepiaShader, VignetteShader } from 'app/arca/materials'
...
export interface IMapRenderingManager extends IPassManager {
   ...
   *
   * @param options - outline options from the {@link app/arca/datasource-protocol#Theme}.
   */
  updateOutline(options: { thickness: number; color: string; ghostExtrudedPolygons: boolean }): void
}
...
export class MapRenderingManager implements IMapRenderingManager {
  ...
  constructor(
    ...
    antialiasSettings: IMapAntialiasSettings | undefined = {
      msaaEnabled: false,
    }
  ) {
  ...
  render(
    ...
    const usePostEffects =
      this.bloom.enabled || this.outline.enabled || this.vignette.enabled || this.sepia.enabled
