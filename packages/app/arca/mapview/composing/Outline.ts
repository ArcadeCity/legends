 ...
 */
import {
  EdgeMaterial, ExtrusionFeatureDefs, FadingFeature, MixinShaderProperties, UniformsType
} from 'app/arca/materials'
import { chainCallbacks } from 'app/arca/utils'
  ...
  private restoreOriginalMaterial(object: THREE.Object3D) {
    ...
    if (Array.isArray((object as THREE.Mesh).material)) {
      for (
        ...
        ;((object as THREE.Mesh).material as THREE.Material[])[i] =
          this.m_originalMaterials[((object as THREE.Mesh).material as THREE.Material[])[i].uuid]
      }
    } else {
