...
import { GeometryType, getFeatureId, Technique } from 'app/arca/datasource-protocol'
import { OrientedBox3, TileKey } from 'app/arca/geoutils'
...
export class PickHandler {
  ...
  constructor(
    ...
    this.m_pickingRaycaster = new PickingRaycaster(mapView.renderer.getSize(new THREE.Vector2()))
  }
  ...
  private getIntersectedTiles(rayCaster: THREE.Raycaster): Array<{ tile: Tile; distance: number }> {
    ...
    tiles.sort((lhs: { tile: Tile; distance: number }, rhs: { tile: Tile; distance: number }) => {
