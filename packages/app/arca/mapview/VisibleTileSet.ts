 ...
 */
import { ViewRanges } from '../datasource-protocol'
import { GeoCoordinates, Projection, ProjectionType, TileKey, TileKeyUtils, TilingScheme } from '../geoutils'
import { assert, LRUCache, MathUtils, TaskQueue } from '../utils'
...
export class VisibleTileSet {
    ...
    for (const { dataSource, visibleTileKeys } of visibleTileKeysResult.tileKeys) {
      ...
      this.dataSourceTileList.push({
        ...
        allVisibleTileLoaded:
          visibleResult.allDataSourceTilesLoaded && dependentResult.allDataSourceTilesLoaded,
   ...
   */
  getRenderedTile(dataSource: DataSource, tileKey: TileKey, offset: number = 0): Tile | undefined {
  ...
  /**
   * Dispose tiles that are marked for removal by {@link app/arca/lrucache#LRUCache} algorithm.
   */
  disposePendingTiles() {
  ...
  private processVisibleTiles(
    ...
    for (
      let i = 0;
      i < visibleTileKeys.length && visibleTiles.length < this.options.maxVisibleDataSourceTiles;
      i++
    ) {
  ...
  private populateRenderedTiles() {
    this.dataSourceTileList.forEach((renderListEntry) => {
      ...
      for (const tileKeyCode of incompleteTiles) {
        if (searchDirection === SearchDirection.BOTH || searchDirection === SearchDirection.UP) {
        ...
        if (searchDirection === SearchDirection.BOTH || searchDirection === SearchDirection.DOWN) {
