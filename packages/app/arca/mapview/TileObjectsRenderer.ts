 ...
 */
import {
  Expr, getFeatureId, getPropertyValue, IndexedTechnique, MapEnv, Pickability
...
export class TileObjectRenderer {
  ...
  private getStencilValue(renderOrder: number) {
    return (
      this.m_renderOrderStencilValues.get(renderOrder) ?? this.allocateStencilValue(renderOrder)
    )
  }
  ...
  private processTileObject(
    ...
    if (mapObjectAdapter) {
      ...
      if (!mapObjectAdapter.isVisible() && !(mapObjectAdapter.pickability === Pickability.all)) {
        return false
      }
    }
    return true
  ...
  private processTileObjectFeatures(
    ...
    objInfos.forEach((properties, featureIndex) => {
      ...
      if (Expr.isExpr(technique.enabled)) {
        ...
        // create a new {@link app/arca/datasource-protocol#Env} that can be used
        // to evaluate expressions that access the feature state.
        const $state = featureState ? new MapEnv(featureState) : null

        const parentEnv =
          typeof properties === 'object' ? new MapEnv(properties, this.m_env) : this.m_env
