 ...
 */
import { PoiStackMode, PoiTableDef, PoiTableEntryDef, PoiTableRef } from 'app/arca/datasource-protocol'
        ...
        for (const tableEntry of jsonPoiTable.poiList) {
          if (PoiTableEntry.verifyJSON(tableEntry)) {
            ...
            if (!this.useAltNamesForKey) {
              ...
              if (newPoiEntry.altNames !== undefined && newPoiEntry.altNames.length > 0) {
            ...
            logger.warn(
              `load: Invalid entry in POI table '${poiTableUrl}' : ${JSON.stringify(tableEntry)}`
            )
          }
        }
      }
 ...
 * can be defined in the {@link app/arca/datasource-protocol#Theme} sfile.
 */
export class PoiTableManager {
   ...
   * {@link app/arca/datasource-protocol#Theme}.
   *
   * @remarks
   * Note that duplicate names of {@link PoiTable}s in the
   * {@link app/arca/datasource-protocol#Theme} will lead to inaccessible {@link PoiTable}s.
   *
   * @param poiTables - {@link app/arca/datasource-protocol#PoiTableRef[]}
   *                containing all {@link PoiTable}s to load.
   *
   * @returns Resolved once all the {@link PoiTable}s in
   *          the {@link app/arca/datasource-protocol#Theme} have been loaded.
   */
   ...
   * are specified in the {@link app/arca/datasource-protocol#Theme}.
   *
   * @remarks
