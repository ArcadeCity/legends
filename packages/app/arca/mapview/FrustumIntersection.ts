 ...
 */
import {
  OrientedBox3, Projection, ProjectionType, TileKey, TileKeyUtils, TilingScheme
...
export class FrustumIntersection {
  ...
  compute(
    ...
    const useElevationRangeSource: boolean =
      elevationRangeSource !== undefined && elevationRangeSource.getTilingScheme() === tilingScheme
    ...
    return {
      ...
      calculationFinal: cache.calculationFinal,
    }
  }
  ...
  private getTileKeyEntry(
    ...
    cache: {
      calculationFinal: boolean
      tileBounds: OrientedBox3 | THREE.Box3
    },
    ...
    if (elevationRangeSource !== undefined) {
      ...
      cache.calculationFinal =
        cache.calculationFinal && range.calculationStatus === CalculationStatus.FinalPrecise
    }
   ...
   */
  private computeTileAreaAndDistance(tileBounds: THREE.Box3 | OrientedBox3): {
    area: number
    distance: number
  } {
