 ...
 */
import {
  EarthConstants, GeoBox, GeoCoordinates, GeoCoordinatesLike, GeoCoordLike, MathUtils, OrientedBox3,
  Projection, ProjectionType, sphereProjection, TileKeyUtils, Vector2Like, Vector3Like
} from '../geoutils'
import { assert, DOMUtils, LoggerManager } from '../utils'
 ...
 * {@link app/arca/map-controls#MapControls} never matches
 * exactly the target due to the precision loss caused by the
 * conversion from zoom level to camera distance (done in
 * {@link (MapView.lookAt:WITH_PARAMS)} and {@link app/arca/map-controls#MapControls})
...
export namespace MapViewUtils {
  ...
  function computeClampedDeltaTilt(
    ...
    const clampedDeltaTilt =
      MathUtils.clamp(deltaTilt + rotationCenterTilt, 0, Math.min(maxTilt, maxRotationTiltAngle)) -
      rotationCenterTilt
  ...
  export function getTargetAndDistance(
    ...
    // infinity.
    const target =
      cameraPitch < MAX_TILT_RAD ? getWorldTargetFromCamera(camera, projection, elevation) : null
    if (target !== null) {
  ...
  /**
   * Returns the {@link app/arca/geoutils#GeoCoordinates} of the camera,
   ...
   * @internal
   *
   * Return `GeoPoints` bounding {@link app/arca/geoutils#GeoBox}
   * applicable for {@link getFitBoundsDistance}.
   *
   * @returns {@link app/arca/geoutils#GeoCoordinates} set that covers `box`
   */
  ...
  export function panCameraAroundGlobe(
    ...
    cache.quaternions[0].setFromUnitVectors(fromWorld.normalize(), toWorld.normalize()).invert()
   ...
   * @param projection - The {@link app/arca/geoutils#Projection} used to
  ...
  export function extractCameraTilt(camera: THREE.Camera, projection: Projection): number {
    ...
    if (projection.type === ProjectionType.Planar) {
      ...
      if (targetGeoCoords !== null) {
        ...
        logger.warn('MapView camera is pointing in the void, using maxTilt: ', MAX_TILT_RAD)
        return MAX_TILT_RAD
      }
    }
  }
   ...
   *
   * @param projection - The {@link app/arca/geoutils#Projection} used when
   ...
   */
  export function getCameraFrustumPlanes(camera: THREE.PerspectiveCamera): {
    left: number
    right: number
    top: number
    bottom: number
  ...
  /**
   * @deprecated Use {@link app/arca/utils#DOMUtils.getBrowserLanguages}
   */
...
export namespace TileOffsetUtils {
  /**
   * @deprecated Use {@link app/arca/geoutils#TileKeyUtils.getKeyForTileKeyAndOffset}.
   */
  ...
  /**
   * @deprecated Use {@link app/arca/geoutils#TileKeyUtils.getParentKeyFromKey}.
   */
