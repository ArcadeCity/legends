...
import {
  Env, FontCatalogConfig, MapEnv, PostEffects, TextStyleDefinition, Theme, Value, ViewRanges
} from 'app/arca/datasource-protocol'
import {
  EarthConstants, GeoBox, GeoBoxExtentLike, GeoCoordinates, GeoCoordLike, GeoPolygon, isGeoBoxExtentLike,
  isGeoCoordinatesLike, isVector3Like, mercatorProjection, OrientedBox3, Projection, ProjectionType,
  TilingScheme, Vector3Like
} from 'app/arca/geoutils'
import {
  assert, getOptionValue, LoggerManager, LogLevel, PerformanceTimer, TaskQueue, UriResolver
} from 'app/arca/utils'
...
import { DEFAULT_FOV_CALCULATION, FovCalculation, MAX_FOV_DEG, MIN_FOV_DEG } from './FovCalculation'
...
if (isProduction) {
  ...
  console.log('not minimizing log level')
...
export class MapView extends EventDispatcher {
  ...
  constructor(options: MapViewOptions) {
    ...
    // HAX: If we pass in a renderer, use that. Otherwise, create new.
    if (this.m_options.renderer !== undefined) {
      this.m_renderer = this.m_options.renderer
      this.m_options.canvas = this.m_options.renderer.domElement
    ...
    if (this.m_options.camera) {
      this.m_camera = this.m_options.camera
      this.m_camera.fov = this.m_options.fovCalculation!.fov
      ...
      console.log('Camera set to passed-in camera')
      // console.log('Fuck yes we passed in the camera WHICH IS NOW', this.m_camera)
    } else {
    ...
    if (this.m_options.scene) {
      this.m_scene = this.m_options.scene
      console.log('and the SCENE')
      console.log('btw we at zoomlevel', this.zoomLevel)
    }
    ...
    // console.log('skipping camera scene looks like we got already')
    ...
    console.log('Constructed with worldcenter:', this.worldCenter)
    console.log("Constructed with target's geo position:", this.target)
    console.log('Constructed with camera:', this.camera.position)

    this.renderLabels = false
    // const wat = useStore.getState()
    // console.log('r3f store maybe:', wat)
  }
  ...
  private updateCameras(viewRanges?: ViewRanges) {
    // console.log('updateCameras w viewRanges', viewRanges)
    ...
    // console.log('What the fuck is this?', cache.vector2[0])
    // const { width, height } = this.m_renderer.getSize(cache.vector2[0])

    const width = 414
    const height = 804

    // console.log('Temporarily hardcoded width and height:', width, height)

    // temporary hardcode of this rofl
    ...
    // console.log('Updated cameras to look at', this.target.latitude, this.target.longitude)
    // console.log('Yes, made it to here.')
  }
  ...
  private updateLookAtSettings() {
    ...
    // console.log('m_zoomLevel', this.m_zoomLevel)
    // console.log('m_targetDistance', this.m_targetDistance)
    // console.log('m_targetGeoPos', this.m_targetGeoPos)
  }
  ...
  private render(frameStartTime: number): void {
    ...
    // const worldposition = this.projection.projectPoint(this.m_targetGeoPos, new THREE.Vector3())
    // console.log(worldposition)

    // make a gigantic sphere at the target position
    ...
    // console.log('made a fucken sphere')

    // this.m_targetWorldPos.copy(this.m_targetGeoPos)
    // this.projection.projectPoint(this.m_targetWorldPos, this.m_targetWorldPos)

    // console.log(this.target.)
    // console.log('# of visible tiles:', this.m_visibleTiles.dataSourceTileList.length)

    // console.log the mortoncode of each visible tile
    // this.m_visibleTiles.dataSourceTileList.forEach((dataSourceTileList) => {
    //   let wat = ''
    //   dataSourceTileList.visibleTiles.forEach((tile) => {
    //     wat += `${tile.tileKey.mortonCode()} `
    //   })
    //   console.log(wat)
    // })
    ...
    // console.log('m_rteCamera:', this.m_rteCamera)
  ...
  private setupCamera() {
    ...
    // ### move & customize
    const { width, height } = this.getCanvasClientSize()
    console.log('RESIZING TO WIDTH AND HEIGHT:', width, height)
