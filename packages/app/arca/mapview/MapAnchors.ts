...
import { StylePriority } from 'app/arca/datasource-protocol'
import {
  GeoCoordinates, GeoCoordLike, isGeoCoordinatesLike, isVector3Like, Projection, Vector3Like
} from 'app/arca/geoutils'
import * as THREE from 'three'

/**
 * An interface describing [[THREE.Object3D]]s anchored on
 * given {@link app/arca/geoutils#GeoCoordinates}.
 *
 * @remarkks
 * @example
 * Example:
...
export type MapAnchor<T extends THREE.Object3D = THREE.Object3D> = T & {
  /**
   * The position of this [[MapAnchor]] in {@link app/arca/geoutils#GeoCoordinates}.
   * @deprecated Use [[anchor]] instead.
   */
  geoPosition?: GeoCoordinates

  /**
   * The anchor of this Object3D in {@link app/arca/geoutils#GeoCoordinates}
    ...
    this.m_anchors.forEach((mapAnchor: MapAnchor) => {
      if (mapAnchor.styleSet !== undefined) {
        const priority = this.m_priorities?.findIndex(
          (entry) => entry.group === mapAnchor.styleSet && entry.category === mapAnchor.category
        )
