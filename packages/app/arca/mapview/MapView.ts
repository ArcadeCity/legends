import {
  Env, FontCatalogConfig, MapEnv, PostEffects, TextStyleDefinition, Theme, ViewRanges
} from 'app/arca/datasource-protocol'
import { ECSYThreeEntity, ECSYThreeWorld } from 'app/arca/ecs-three'
import {
  EarthConstants, GeoBox, GeoBoxExtentLike, GeoCoordinates, GeoCoordLike, GeoPolygon, isGeoBoxExtentLike,
  isGeoCoordinatesLike, mercatorProjection, OrientedBox3, Projection, ProjectionType, TilingScheme,
  Vector3Like
} from 'app/arca/geoutils'
import { CameraAnimationBuilder, CameraKeyTrackAnimation, ControlPoint } from 'app/arca/map-controls'
import { Rotating } from 'app/arca/realgame/components/Rotating'
import {
  assert, getOptionValue, LoggerManager, PerformanceTimer, TaskQueue, UriResolver
} from 'app/arca/utils'
...
export interface MapViewOptions {
   ...
   * ```
   *
   * @see {@link app/arca/utils#UriResolver}
   * @See {@link app/arca/utils#PrefixMapUriResolver}
   */
...
export class MapView extends EventDispatcher {
   ...
   * | ------               | ------      | --------
   * | {@link app/arca/geoutils#GeoBox}           | _defined_   | `params.target` is used
   * | {@link app/arca/geoutils#GeoBox}           | `undefined` | `bounds.center` is used as new `target`
   * | {@link app/arca/geoutils#GeoBoxExtentLike} | `undefined` | current `MapView.target` is used
   * | {@link app/arca/geoutils#GeoBoxExtentLike} | _defined_   | `params.target` is used
   ...
   *  - if the tilt is high and projection is {@link app/arca/geoutils#sphereProjection}`
...
export interface LookAtParams {
   ...
   *
   * * if `bounds` is {@link app/arca/geoutils#GeoBox}, then `lookAt`
   *   use {@link LookAtParams.target} or `bounds.target` and
   *   ensure whole box is visible
   *
   * * if `bounds` is {@link app/arca/geoutils#GeoPolygon}, then `lookAt`
   *   use `bounds.getCentroid()` and ensure whole polygon is visible
   *
   * * if `bounds` is {@link app/arca/geoutils#GeoBoxExtentLike},
