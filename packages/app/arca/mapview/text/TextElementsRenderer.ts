 ...
 */
import { FontCatalogConfig, LineMarkerTechnique, TextStyleDefinition } from 'app/arca/datasource-protocol'
import { TileKey, Vector3Like } from 'app/arca/geoutils'
import {
  AdditionParameters, DEFAULT_TEXT_CANVAS_LAYER, FontCatalog, TextBufferAdditionParameters,
  TextBufferCreationParameters, TextCanvas, TextLayoutStyle, TextRenderStyle
} from 'app/arca/text-canvas'
import { assert, LoggerManager, LogLevel, Math2D, MathUtils, PerformanceTimer } from 'app/arca/utils'
...
import {
  checkReadyForPlacement, computeViewDistance, getMaxViewDistance, getWorldPosition, isPathLabelTooSmall,
  placeIcon, PlacementResult, placePathLabel, placePointLabel, pointToPlaneDistance, PrePlacementResult
} from './Placement'
...
import { initializeDefaultOptions, TextElementsRendererOptions } from './TextElementsRendererOptions'
...
function shouldRenderPointText(
  ...
  const visibleInZoomLevel =
    poiInfo === undefined ||
    MathUtils.isClamped(viewState.zoomLevel, poiInfo.textMinZoomLevel, poiInfo.textMaxZoomLevel)
...
function shouldRenderPoiText(labelState: TextElementState, viewState: ViewState) {
  ...
  return (
    !viewState.cameraIsMoving || poiInfo === undefined || poiInfo.renderTextDuringMovements === true
  )
}
...
export class TextElementsRenderer {
  ...
  placeText(dataSourceTileList: DataSourceTileList[], time: number) {
    ...
    const updateTextElements =
      this.m_cacheInvalidated || tileTextElementsChanged || this.m_viewState.renderedTilesChanged
  ...
  private async addTextCanvas(fontCatalogConfig: FontCatalogConfig): Promise<void> {
    ...
    if (this.m_textCanvases.has(fontCatalogConfig.name)) {
      ...
      const newLoadPromise = loadFontCatalog(fontCatalogConfig, catalogCallback, errorCallback)
        .then(() => {
          --this.m_loadPromisesCount
        })
        .catch((error) => {
  ...
  private addPathLabel(
    ...
    labelState.setViewDistance(
      computeViewDistance(pathLabel, undefined, this.m_viewState.worldCenter, this.m_cameraLookAt)
    )
