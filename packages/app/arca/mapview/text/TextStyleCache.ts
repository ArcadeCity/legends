 ...
 */
import {
  ColorUtils, getPropertyValue, isPoiTechnique, LineMarkerTechnique, MapEnv, PlacementToken, PoiTechnique,
  TextStyleDefinition, TextTechnique
} from 'app/arca/datasource-protocol'
import {
  DefaultTextStyle, FontStyle, FontUnit, FontVariant, HorizontalAlignment, HorizontalPlacement,
  resolvePlacementAndAlignment, TextCanvas, TextLayoutParameters, TextLayoutStyle, TextPlacement,
  TextPlacements, TextRenderParameters, TextRenderStyle, VerticalAlignment, VerticalPlacement, WrappingMode
} from 'app/arca/text-canvas'
import { getOptionValue, LoggerManager } from 'app/arca/utils'
 ...
 * {@link app/arca/datasource-protocol#Theme}'s TextStyle.
 */
...
export class TextStyleCache {
  ...
  /**
   * Retrieves a {@link TextElementStyle} for {@link app/arca/datasource-protocol#Theme}'s
   * [[TextStyle]] id.
   */
  ...
  /**
   * Gets the appropriate {@link app/arca/text-canvas#TextRenderStyle}
  ...
  /**
   * Create the appropriate {@link app/arca/text-canvas#TextLayoutStyle}
   * to use for a label. Depends heavily on the label's
   * [[Technique]] and the current zoomLevel.
   *
  ...
  createLayoutStyle(
    ...
    const textPlacements = isPoiTechnique(technique)
      ? (getPropertyValue((technique as PoiTechnique).placements, discreteZoomEnv) as string | null)
      : null
    ...
    const layoutParams = {
      tracking:
        getPropertyValue(technique.tracking, discreteZoomEnv) ?? defaultLayoutParams.tracking,
      leading: getPropertyValue(technique.leading, discreteZoomEnv) ?? defaultLayoutParams.leading,
      maxLines:
        getPropertyValue(technique.maxLines, discreteZoomEnv) ?? defaultLayoutParams.maxLines,
      lineWidth:
        getPropertyValue(technique.lineWidth, discreteZoomEnv) ?? defaultLayoutParams.lineWidth,
      canvasRotation:
  ...
  private initializeTextCanvas(style: TextElementStyle, textCanvases: TextCanvases): void {
    ...
    if (style.textCanvas === undefined) {
      if (style.fontCatalog !== undefined && style.fontCatalog !== DEFAULT_FONT_CATALOG_NAME) {
        logger.warn(
  ...
  private createTextElementStyle(style: TextStyleDefinition, styleName: string): TextElementStyle {
    const { horizontalAlignment, verticalAlignment, placements } = parseAlignmentAndPlacements(
      style.hAlignment,
      style.vAlignment,
      style.placements
    ...
    return {
      ...
      renderParams: {
        ...
        color: style.color !== undefined ? ColorCache.instance.getColor(style.color) : undefined,
        backgroundColor:
