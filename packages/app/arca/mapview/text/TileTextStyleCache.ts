...
import {
  IndexedTechniqueParams, LineMarkerTechnique, PoiTechnique, TextTechnique
} from 'app/arca/datasource-protocol'
import { TextLayoutStyle, TextRenderStyle } from 'app/arca/text-canvas'
  ...
  getRenderStyle(
    ...
    if (style === undefined) {
      style = this.textRenderStyles[technique._index] =
        this.tile.mapView.textElementsRenderer.styleCache.createRenderStyle(this.tile, technique)
    }
    return style
  }
  ...
  getLayoutStyle(
    ...
    if (style === undefined) {
      style = this.textLayoutStyles[technique._index] =
        this.tile.mapView.textElementsRenderer.styleCache.createLayoutStyle(this.tile, technique)
    }
    return style
  }
}
