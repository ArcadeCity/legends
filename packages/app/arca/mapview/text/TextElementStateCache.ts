...
import { assert, LoggerManager, LogLevel } from 'app/arca/utils'
...
const logger = LoggerManager.instance.create('TextElementsStateCache', {
  level: LogLevel.Log,
})
...
export class TextElementStateCache {
  ...
  update(time: number, disableFading: boolean, findReplacements: boolean, zoomLevel: number) {
    const replaceCallback = findReplacements ? this.replaceElement.bind(this, zoomLevel) : undefined
