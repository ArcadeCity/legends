...
import { EarthConstants, sphereProjection } from 'app/arca/geoutils'
...
export class SphereHorizon {
  ...
  constructor(
    ...
    this.m_matrix = new THREE.Matrix4().makeBasis(xAxis, yAxis, zAxis).setPosition(horizonCenter)
  ...
  private computeIntersections() {
    ...
    for (let side = CanvasSide.Bottom; side < 4; side++) {
      ...
      this.m_intersections.push(
        sideIntersections.filter((val, i) => val !== undefined && !hasCorners[i]) as number[]
      )
    }
  }
   ...
   */
  private computeLRIntersections(yBottom: number, sideFov: number): [number, number?] | undefined {
