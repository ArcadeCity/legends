 ...
 */
import {
  Attachment, BaseTechniqueParams, BufferAttribute, CallExpr, DecodedTile, Env, Expr, ExprEvaluatorContext,
  ExtrudedPolygonTechnique, FillTechnique, Geometry, GeometryKindSet, getArrayConstructor, getPropertyValue,
  IndexedTechnique, InterleavedBufferAttribute, isCirclesTechnique, isExtrudedLineTechnique,
  isExtrudedPolygonTechnique, isFillTechnique, isJsonExpr, isLineMarkerTechnique, isLineTechnique,
  isPoiTechnique, isSegmentsTechnique, isSolidLineTechnique, isSquaresTechnique, isStandardTechnique,
  isTerrainTechnique, isTextTechnique, MakeTechniqueAttrs, MapEnv, needsVertexNormals, OperatorDescriptor,
  Pickability, SolidLineTechnique, StandardExtrudedLineTechnique, Technique, TerrainTechnique,
  TextPathGeometry, transientToPickability, Value
} from 'app/arca/datasource-protocol'
import { EarthConstants, ProjectionType } from 'app/arca/geoutils'
import {
  EdgeMaterial, EdgeMaterialParameters, ExtrusionFeature, FadingFeature, hasExtrusionFeature,
  isHighPrecisionLineMaterial, MapMeshDepthMaterial, MapMeshStandardMaterial, setDisplacementMapToMaterial,
  setShaderMaterialDefine, SolidLineMaterial
} from 'app/arca/materials'
import { assert, LoggerManager } from 'app/arca/utils'
import * as THREE from 'three'
import {
  applyBaseColorToMaterial, buildMetricValueEvaluator, buildObject, createMaterial, getBufferAttribute,
  usesObject3D
} from '../DecodedTileHelpers'
import { createDepthPrePassMesh, isRenderDepthPrePassEnabled, setDepthPrePassStencil } from '../DepthPrePass'
...
/**
 * Support class to create geometry for a {@link Tile} from a {@link app/arca/datasource-protocol#DecodedTile}.
 * @internal
 */
export class TileGeometryCreator {
  ...
  prepareTextPaths(
    ...
    while (newPaths.length > 0) {
      ...
      if (!isTextTechnique(technique) || (textFilter !== undefined && !textFilter(technique))) {
        continue
      }
    ...
    for (const attachment of this.getAttachments(decodedTile)) {
      ...
      for (let groupIndex = 0; groupIndex < groupCount; ) {
        ...
        const extrusionAnimationEnabled: boolean =
          animatedExtrusionHandler?.setAnimationProperties(technique, discreteZoomEnv) ?? false
        ...
        if (isSolidLineTechnique(technique)) {
          ...
          if (technique.clipping === true && tile.projection.type === ProjectionType.Planar) {
        ...
        registerTileObject(tile, object, techniqueKind, {
          technique,
          pickability: hasSolidLinesOutlines ? Pickability.transient : techniquePickability,
        })
        objects.push(object)

        // Add the extruded polygon edges as a separate geometry.
        if (isExtrudedPolygonTechnique(technique) && attachment.info.edgeIndex !== undefined) {
          // When the source geometry is split in groups, we
          // should create objects with an array of materials.
          ...
          MapMaterialAdapter.create(edgeMaterial, {
            ...
            lineWidth: (frameMapView: MapAdapterUpdateEnv) => {
              ...
              const value = getPropertyValue(buildingTechnique.lineWidth, frameMapView.env)
        ...
        if (hasSolidLinesOutlines) {
          ...
          outlineObj.renderOrder =
            (getPropertyValue(outlineTechnique.secondaryRenderOrder, mapView.env) ?? 0) - 0.0000001
  ...
  private getPolygonFadingParams(
    ...
    const lineFadeNear =
      technique.lineFadeNear !== undefined
        ...
        : fadeNear
    const lineFadeFar =
      technique.lineFadeFar !== undefined ? getPropertyValue(technique.lineFadeFar, env) : fadeFar
