...
import { GeometryType, getFeatureId } from 'app/arca/datasource-protocol'
import { assert, LoggerManager } from 'app/arca/utils'
import * as THREE from 'three'
import { Tile, TileFeatureData } from '../Tile'
import {
  BufferedGeometryLineAccessor, BufferedGeometryObject3dAccessor, IGeometryAccessor, ILineAccessor,
  IndexedBufferedGeometryLineAccessor, IObject3dAccessor, isLineAccessor, isObject3dAccessor
} from './TileGeometry'
...
export class TileDataAccessor {
  ...
  protected visitObject(object: THREE.Object3D): void {
    const featureData: TileFeatureData | undefined =
      object.userData !== undefined ? (object.userData.feature as TileFeatureData) : undefined
  ...
  protected visitMesh(meshObject: THREE.Mesh, featureData: TileFeatureData): void {
    ...
    for (let featureIndex = 0; featureIndex < objInfos.length; featureIndex++) {
      ...
      featureEnd =
        featureIndex < starts.length - 1 ? starts[featureIndex + 1] : geometryAccessor.getCount()
      ...
      switch (geometryType) {
        ...
        case GeometryType.Object3D:
          ...
          this.visitor.visitObject3D(featureId, geometryAccessor as any as IObject3dAccessor)
          break
        default:
          logger.warn('#visitObject: invalid geometryType')
      }
