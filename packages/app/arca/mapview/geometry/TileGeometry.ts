...
import { GeometryType } from 'app/arca/datasource-protocol'
import { reconstructLineWidth } from 'app/arca/lines'
...
export abstract class BufferedGeometryAccessorBase implements IGeometryAccessor {
  ...
  constructor(
    ...
    if (!this.position) {
      logger.warn(
        'BufferedGeometryAccessor#constructor: BufferGeometry has no position ' + 'attribute'
      )
    }
  ...
  constructor(
    ...
    if (!this.indices) {
      ...
      if (!(this.indices instanceof Uint32Array)) {
        logger.warn(
          'IndexedBufferedGeometryAccessor#constructor: BufferGeometry index ' + 'has wrong type'
        )
