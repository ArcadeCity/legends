...
import { OrientedBox3 } from 'app/arca/geoutils'
import { SolidLineMaterial } from 'app/arca/materials'
...
function attributeChanged(
  ...
  return (
    attrInfo === undefined ||
    attrInfo.data !== data ||
    ((attribute as THREE.BufferAttribute).isBufferAttribute && attrInfo.version !== data.version)
  )
}
...
function intersectFeature(
  ...
  for (let i = beginIdx; i < endIdx; i += SEGMENT_STRIDE) {
    ...
    if (
      !intersectLine(localRay, line, vExt, lHalfWidth, lHalfWidthSq, plane, interPlane, interLine)
    ) {
      continue
    }
