...
export class FontCatalog {
  ...
  get isLoading(): boolean {
    return (
      this.m_loadingJson.size > 0 || this.m_loadingPages.size > 0 || this.m_loadingGlyphs.size > 0
    )
  }
  ...
  async loadCharset(input: string, style: TextRenderStyle): Promise<GlyphData[]> {
    ...
    const shouldTransform =
      style.fontVariant === FontVariant.AllCaps || style.fontVariant === FontVariant.SmallCaps
  ...
  getGlyphs(
    ...
    for (const character of input) {
      ...
      for (const char of transformedCharacter) {
        ...
        if (glyphData !== undefined && (!glyphData.isReplacement || this.showReplacementGlyphs)) {
