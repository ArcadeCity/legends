 ...
 */
import { getStyles, ITileDecoder, OptionsMap, Theme, TileInfo } from 'app/arca/datasource-protocol'
import { TileKey, TilingScheme } from 'app/arca/geoutils'
import {
  ConcurrentDecoderFacade, CopyrightInfo, CopyrightProvider, DataSource, DataSourceOptions, ThemeLoader, Tile,
  TileLoaderState
} from 'app/arca/mapview'
import { ILogger, LoggerManager } from 'app/arca/utils'
...
export class TileFactory<TileType extends Tile> {
   ...
   */
  constructor(
    private readonly m_modelConstructor: new (dataSource: DataSource, tileKey: TileKey) => TileType
  ) {}
...
export class TileDataSource<TileType extends Tile = Tile> extends DataSource {
   ...
   *
   * Applies new {app/arca/datasource-protocol StyleSet} and definitions from theme only
