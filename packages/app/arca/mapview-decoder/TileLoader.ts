...
export class TileLoader extends BaseTileLoader {
  ...
  protected loadImpl(
    ...
    // If tile zoomlevel is 7 or less, get the tile from our local cache.
    if (USECACHEDTILES && this.tileKey.level <= 7) {
      // console.log('Grabbing tilekey from cache:', this.tileKey)

      fetch('/api/fetchCachedTile', {
        method: 'POST',
        body: JSON.stringify({
        ...
        .then((payload) => {
          // console.log(
          //   `[cache] Fetched payload of length ${payload.byteLength} - zoomlevel: ${this.tileKey.level}`
          // )
      ...
      // If tile zoomlevel is greater than 7, get the tile from the data provider.

      // console.log('Grabbing tilekey from data provider:', this.tileKey)
      this.dataProvider
  ...
  private onLoaded(
    ...
    if (byteLength === 0 || (payload.constructor === Object && Object.keys(payload).length === 0)) {
      // Object is empty
      this.onDecoded(
        {
          geometries: [],
