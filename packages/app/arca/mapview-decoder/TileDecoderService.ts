...
import {
  addBuffersToTransferList, BufferAttribute, getProjection, InterleavedBufferAttribute, ITileDecoder,
...
export class TileDecoderService extends WorkerService {
  ...
  private async handleDecodeTileRequest(
    ...
    const transferBufferAttribute = (attribute?: BufferAttribute | InterleavedBufferAttribute) => {
      if (
        attribute !== undefined &&
  ...
  private handleTileInfoRequest(
    ...
    return this.m_decoder.getTileInfo(request.data, tileKey, projection).then((tileInfo) => {
      const transferList: ArrayBuffer[] =
        tileInfo !== undefined && tileInfo.transferList !== undefined ? tileInfo.transferList : []
      return {
        response: tileInfo,
        transferList,
      }
