...
import { GeoJson, ITiler, WorkerServiceProtocol } from 'app/arca/datasource-protocol'
import { TileKey } from 'app/arca/geoutils'
import { ConcurrentTilerFacade } from 'app/arca/mapview'
...
/**
 * GeoJson {@link app/arca/mapview-decoder@DataProvider}.
 *
 * @remarks
...
export class GeoJsonDataProvider extends DataProvider {
  ...
  async connect(): Promise<void> {
    try {
      ...
      if (WorkerServiceProtocol.isUnknownServiceError(error) && !missingTilerServiceInfoEmitted) {
        logger.info(
