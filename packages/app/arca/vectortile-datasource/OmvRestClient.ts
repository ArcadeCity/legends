...
import { DataProvider } from 'app/arca/mapview-decoder'
import { ITransferManager, TransferManager } from 'app/arca/transfer-manager'
...
export interface OmvRestClientParameters {
   ...
   *  - `{z}` - zoom level of tile, @see {@link app/arca/geoutils#TileKey.level}
   *  - `{x}` - horizontal coordinate of tile (column number),
   *            see {@link app/arca/geoutils#TileKey.column}
   *  - `{y}` - vertical coordinate of Tile (row number),
   *            see {@link app/arca/geoutils#TileKey.row}
   *
...
export class OmvRestClient extends DataProvider {
  ...
  constructor(readonly params: OmvRestClientParameters) {
    ...
    this.downloadManager =
      params.downloadManager === undefined ? TransferManager.instance() : params.downloadManager
  ...
  private dataUrl(tileKey: TileKey): string {
    ...
    let path = [`/${tileKey.level}`, tileKey.column, tileKey.row].join(
      this.params.apiFormat === APIFormat.XYZSpace || this.params.apiFormat === APIFormat.XYZJson
        ? '_'
        : '/'
    )
    switch (this.params.apiFormat) {
