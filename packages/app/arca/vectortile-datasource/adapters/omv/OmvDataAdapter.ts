...
import { MapEnv, StyleSetEvaluator, Value, ValueMap } from 'app/arca/datasource-protocol/index-decoder'
import { TileKey } from 'app/arca/geoutils'
import { assert, ILogger, LoggerManager } from 'app/arca/utils'
import Long from 'long' // [CD]
import { ShapeUtils, Vector2, Vector3 } from 'three'
import { DataAdapter } from '../../DataAdapter'
import { DecodeInfo } from '../../DecodeInfo'
import { IGeometryProcessor, ILineGeometry, IPolygonGeometry } from '../../IGeometryProcessor'
import {
  ComposedDataFilter, OmvFeatureFilter, OmvFeatureModifier, OmvGenericFeatureFilter, OmvGenericFeatureModifier
} from '../../OmvDataFilter'
import {
  FeatureModifierId, OmvDecoderOptions, OmvFeatureFilterDescription, OmvGeometryType
} from '../../OmvDecoderDefs'
...
const logger = LoggerManager.instance.create('OmvDataAdapter', {
  enabled: false,
})
...
export class OmvDataAdapter implements DataAdapter, OmvVisitor {
    ...
    if (options.filterDescription !== undefined) {
      if (options.filterDescription !== null) {
        ...
        if (featureModifiersIds !== undefined) {
          ...
          featureModifiers.push(createFeatureModifier(filterDescription, FeatureModifierId.default))
        }
  ...
  visitLayer(layer: com.mapbox.pb.Tile.ILayer): boolean {
    ...
    if (this.m_dataFilter !== undefined && !this.m_dataFilter.wantsLayer(layerName, storageLevel)) {
      return false
    }
