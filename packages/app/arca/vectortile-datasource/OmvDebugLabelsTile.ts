 ...
 */
import { getPropertyValue, isTextTechnique } from 'app/arca/datasource-protocol'
import { TileKey } from 'app/arca/geoutils'
import {
  DataSource, debugContext, TextElement, TextElementType, Tile, TileGeometryCreator
} from 'app/arca/mapview'
import { ContextualArabicConverter, FontUnit, TextLayoutStyle, TextRenderStyle } from 'app/arca/text-canvas'
...
export class OmvDebugLabelsTile extends Tile {
  ...
  loadingFinished() {
    ...
    if (this.textElementGroups.count() > 0) {
      ...
      addObject(this.objects, bluePoints, 0, (geometry: THREE.BufferGeometry): THREE.Object3D => {
        return new THREE.Mesh(geometry, debugBlueCircleMaterial)
      })
      addObject(this.objects, orangePoints, 0, (geometry: THREE.BufferGeometry): THREE.Object3D => {
    ...
    if (this.preparedTextPaths !== undefined) {
      ...
      for (const textPath of this.preparedTextPaths) {
        ...
        if (technique.color !== undefined) {
          colorMap.set(textPath.technique, new THREE.Color(getPropertyValue(technique.color, env)))
        }
          ...
          for (let i = 0; i < textPath.path.length; i += 3) {
            ...
            if (debugTextPathsFull || isRedPoint) {
              ...
              if (debugTextPathsFull) {
                ...
                const label: string =
                  pathIndex % 5 === 0 ? text + ':' + pathIndex : Number(pathIndex).toString()
                const labelElement = new TextElement(
                  ContextualArabicConverter.instance.convert(label),
                  new THREE.Vector3(x + worldOffsetX + centerX, y + centerY, z + centerZ),
                  textRenderStyle,
                  textLayoutStyle,
      ...
      addObject(this.objects, lines, -2, (geometry: THREE.BufferGeometry): THREE.Object3D => {
        return new THREE.LineSegments(geometry, debugMaterial)
      })
      addObject(this.objects, redPoints, 0, (geometry: THREE.BufferGeometry): THREE.Object3D => {
        return new THREE.Mesh(geometry, debugCircleMaterial)
      })
      addObject(this.objects, blackPoints, -1, (geometry: THREE.BufferGeometry): THREE.Object3D => {
