...
import {
  AttrEvaluationContext, AttributeMap, BufferAttribute, composeTechniqueTextureName, DecodedTile,
  evaluateTechniqueAttr, ExtrudedPolygonTechnique, FillTechnique, Geometry, GeometryType, getFeatureId,
  getFeatureText, Group, IndexedTechnique, InterleavedBufferAttribute, isExtrudedLineTechnique,
  isExtrudedPolygonTechnique, isFillTechnique, isLabelRejectionLineTechnique, isLineMarkerTechnique,
  isLineTechnique, isPoiTechnique, isShaderTechnique, isSolidLineTechnique, isSpecialDashesLineTechnique,
  isStandardTechnique, isTextTechnique, LineMarkerTechnique, PathGeometry, PoiGeometry, PoiTechnique,
  scaleHeight, StyleColor, Technique, TextGeometry, TextPathGeometry, TextTechnique, TextureCoordinateType,
  textureCoordinateType
} from 'app/arca/datasource-protocol'
import { Env, IMeshBuffers, StyleSetEvaluator, Value } from 'app/arca/datasource-protocol/index-decoder'
import {
  clipPolygon, EdgeLengthGeometrySubdivisionModifier, SphericalGeometrySubdivisionModifier, SubdivisionMode
} from 'app/arca/geometry'
import {
  GeoBox, GeoCoordinates, normalizedEquirectangularProjection, ProjectionType, Vector3Like,
  webMercatorProjection
} from 'app/arca/geoutils'
import { LineGroup, triangulateLine } from 'app/arca/lines'
import { ExtrusionFeatureDefs } from 'app/arca/materials'
import { assert, getOptionValue, LoggerManager, Math2D } from 'app/arca/utils'
import earcut from 'earcut'
import * as THREE from 'three'
import { DecodeInfo } from './DecodeInfo'
import { ILineGeometry, IPolygonGeometry } from './IGeometryProcessor'
import { tile2world, webMercatorTile2TargetTile, webMercatorTile2TargetWorld, world2tile } from './OmvUtils'
...
export class VectorTileDataEmitter {
  ...
  processPointFeature(
    ...
    for (const technique of techniques) {
      ...
      const shouldCreateTextGeometries = isTextTechnique(technique) || isPoiTechnique(technique)
      ...
      for (const pos of tilePositions) {
        ...
        // misplaced.
        if (shouldCreateTextGeometries) {
          webMercatorTile2TargetWorld(extents, this.m_decodeInfo, pos, tmpV3, scaleHeights)
        } else {
          webMercatorTile2TargetTile(extents, this.m_decodeInfo, pos, tmpV3, scaleHeights)
        }
  ...
  processLineFeature(
      ...
      if (hasIndividualLineSegments) {
        ...
        if (pointCount > 1) {
          ...
          for (let i = 0; i < pointCount - 1; i++) {
            ...
            webMercatorTile2TargetWorld(extents, this.m_decodeInfo, pos1, tmpV3, scaleHeights)
            worldLine.push(tmpV3.x, tmpV3.y, tmpV3.z)
            webMercatorTile2TargetWorld(extents, this.m_decodeInfo, pos2, tmpV4, scaleHeights)
    ...
    for (const technique of techniques) {
      ...
      if (isLineTechnique(technique) || isSolidLineTechnique(technique)) {
        const lineGeometry = isLineTechnique(technique) ? this.m_simpleLines : this.m_solidLines
        ...
        if (hasIndividualLineSegments) {
          assert(
            ...
            'OmvDecodedTileEmitter#processLineFeature: ' + 'Internal error - No localLineSegments'
          )
        ...
        if (this.m_options.skipShortLabels) {
          ...
          const minEstimatedLabelLength =
            MIN_AVERAGE_CHAR_WIDTH * text.length * worldUnitsPerPixel * SIZE_ESTIMATION_FACTOR
        ...
        if (isTextTechnique(technique)) {
          ...
          let imageTexture = evaluateTechniqueAttr(context, lineMarkerTechnique.imageTexture)
        ...
        const meshBuffers = this.findOrCreateMeshBuffers(techniqueIndex, GeometryType.ExtrudedLine)
        if (meshBuffers === undefined) {
          continue
        }
        ...
        const techniqueCaps = evaluateTechniqueAttr<string>(context, technique.caps, 'Circle')
        ...
        localLines.forEach((aLine) => {
          ...
          objInfos.push(
            this.m_options.gatherFeatureAttributes ? env.entries : getFeatureId(env.entries)
          )
        })
  ...
  processPolygonFeature(
    ...
    techniques.forEach((technique) => {
      ...
      if (Array.isArray(bbox)) {
        ...
        const geoBox = new GeoBox(new GeoCoordinates(south, west), new GeoCoordinates(north, east))
      ...
      for (const polygon of geometry) {
        ...
        for (let i = 0; i < polygon.rings.length; i++) {
          ...
          if (area !== 0) {
            ...
            rings.push(new Ring(ringCoords, textureCoords, extents, shouldClipPolygons))
          }
        }
      ...
      if (isPolygon) {
        ...
        polygons.forEach((rings) => {
          const lines: number[][] = []
          const offsets: number[][] | undefined = needIndividualLineSegments ? [] : undefined
          rings.forEach((ring) => {
            ...
            let ringLength = 0
            let lastSegmentOffset = 0
            let segmentOffsets: number[] | undefined = needIndividualLineSegments ? [] : undefined
  ...
  private applyPolygonTechnique(
    ...
    let floorHeight = evaluateTechniqueAttr<number>(context, extrudedPolygonTechnique.floorHeight)
    ...
    const scaleHeights = isExtruded && scaleHeight(context, extrudedPolygonTechnique, tileLevel)
    ...
    this.m_meshBuffers.forEach((meshBuffers, techniqueIdx) => {
      ...
      if (meshBuffers.normals.length > 0) {
        ...
        assert(
          ...
          'length of normals buffer is different than the length of the ' + 'position buffer'
        )
      ...
      if (meshBuffers.colors.length > 0) {
        ...
        assert(
          ...
          'length of colors buffer is different than the length of the ' + 'position buffer'
        )
      ...
      if (meshBuffers.extrusionAxis.length > 0) {
        ...
        assert(
          ...
          'length of extrusionAxis buffer is different than the length of the ' + 'position buffer'
        )
