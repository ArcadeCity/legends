 ...
 */
import { GeometryKind, GeometryKindSet } from 'app/arca/datasource-protocol'
import { MapEnv } from 'app/arca/datasource-protocol/index-decoder'
import {
  OmvFeatureFilterDescription, OmvFilterDescription, OmvFilterFeatureAttribute, OmvFilterString,
  OmvGeometryType, OmvLayerFilterDescription
} from './OmvDecoderDefs'
  ...
  constructor(
    ...
    if (options) {
      ...
      this.m_processPolygonsDefault =
        options.processPolygonsDefault !== undefined ? options.processPolygonsDefault : true
    }
  }
  ...
  private addItem(
    ...
    const item: OmvFilterDescription = {
      layerName: {
        value: options.layer,
        match:
          options.matchLayer === undefined ? OmvFilterString.StringMatch.Match : options.matchLayer,
      },
      geometryTypes:
...
export class OmvGenericFeatureFilter implements OmvFeatureFilter {
  ...
  constructor(private readonly description: OmvFeatureFilterDescription) {
    if (this.description.kindsToProcess.length > 0) {
      this.enabledKinds = new GeometryKindSet(this.description.kindsToProcess as GeometryKind[])
    }
    if (this.description.kindsToIgnore.length > 0) {
      this.disabledKinds = new GeometryKindSet(this.description.kindsToIgnore as GeometryKind[])
    }
  }
  ...
  wantsKind(kind: string | string[]): boolean {
    ...
    return (
      !(
        this.disabledKinds !== undefined && this.disabledKinds.hasOrIntersects(kind as GeometryKind)
      ) ||
      (this.enabledKinds !== undefined && this.enabledKinds.hasOrIntersects(kind as GeometryKind))
    )
  }
...
export class ComposedDataFilter implements OmvFeatureFilter {
   ...
   */
  get hasKindFilter() {
    return this.filters.reduce<boolean>((result, filter) => result || filter.hasKindFilter, true)
  }
   ...
   */
  wantsKind(kind: string | string[]): boolean {
    return this.filters.reduce<boolean>((result, filter) => result && filter.wantsKind(kind), true)
  }
}
/**
...
export class OmvGenericFeatureModifier implements OmvFeatureModifier {
  ...
  protected doProcessFeature(
    ...
    if (featureClass && OmvGenericFeatureModifier.matchItems(layer, featureClass, itemsToProcess)) {
      return true
    }

    if (featureClass && OmvGenericFeatureModifier.matchItems(layer, featureClass, itemsToIgnore)) {
      return false
    }
