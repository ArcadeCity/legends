 ...
 */
import { DecoderOptions, OptionsMap, WorkerServiceProtocol } from 'app/arca/datasource-protocol'
import { EarthConstants, TileKey, webMercatorTilingScheme } from 'app/arca/geoutils'
import { DataSourceOptions, Tile } from 'app/arca/mapview'
import { DataProvider, TileDataSource, TileDataSourceOptions, TileFactory } from 'app/arca/mapview-decoder'
import { getOptionValue, LoggerManager } from 'app/arca/utils'
import {
  FeatureModifierId, OmvDecoderOptions, OmvFeatureFilterDescription, VECTOR_TILE_DECODER_SERVICE_TYPE
} from './OmvDecoderDefs'
import { APIFormat, AuthenticationMethod, OmvRestClient, OmvRestClientParameters } from './OmvRestClient'

const logger = LoggerManager.instance.create('VectorTileDataSource')

export interface VectorTileFactory {
  /** Create an instance of {@link app/arca/mapview#Tile} or a subclass. */
...
export interface VectorTileDataSourceParameters
  ...
  /**
   * Optional, custom factory for {@link app/arca/mapview#Tile} instances created
...
export class VectorTileDataSource extends TileDataSource {
  ...
  constructor(private readonly m_params: OmvWithRestClientParams | OmvWithCustomDataProvider) {
    ...
    this.cacheable = true
    this.addGroundPlane = m_params.addGroundPlane === undefined || m_params.addGroundPlane === true
