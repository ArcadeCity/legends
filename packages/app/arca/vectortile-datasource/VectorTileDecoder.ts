 ...
 */
import {
  AttrEvaluationContext, DecodedTile, DecoderOptions, GeometryKind, IndexedTechnique, OptionsMap, ValueMap
} from 'app/arca/datasource-protocol'
import { MapEnv, StyleSetEvaluator } from 'app/arca/datasource-protocol/index-decoder'
import { Projection, TileKey } from 'app/arca/geoutils'
import {
  ThemedTileDecoder, TileDecoderService, WorkerServiceManager
} from 'app/arca/mapview-decoder/index-worker'
import { assert, LoggerManager, PerformanceTimer } from 'app/arca/utils'
...
const logger = LoggerManager.instance.create('VectorTileDecoder', {
  enabled: false,
})
...
export class VectorTileDataProcessor implements IGeometryProcessor {
  ...
  private processFeature(
    ...
    if (this.m_decodedTileEmitter) {
      emitterFunc.apply(this.m_decodedTileEmitter, [layer, extents, geometry, context, techniques])
    }
  }
  ...
  private applyKindFilter(
    ...
    if (this.m_dataFilter !== undefined && this.m_dataFilter.hasKindFilter) {
      techniques = techniques.filter((technique) => {
        const kind = technique.kind === undefined ? defaultKind : (technique.kind as GeometryKind)
...
export class VectorTileDecoder extends ThemedTileDecoder {
  ...
  private m_options: { map: OptionsMap; changed: boolean } = {
    map: {},
    changed: false,
  }
...
export class VectorTileDecoderService {
  ...
  static start() {
    WorkerServiceManager.getInstance().register({
      ...
      factory: (serviceId: string) => TileDecoderService.start(serviceId, new VectorTileDecoder()),
    })
  }
}
