...
import {
  HighPrecisionLineMaterial, HighPrecisionLineMaterialParameters, HighPrecisionPointMaterial,
  isHighPrecisionPointMaterial
} from 'app/arca/materials'
import {
  BufferAttribute, BufferGeometry, Camera, Float32BufferAttribute, InterleavedBuffer,
  InterleavedBufferAttribute, Matrix4, PointsMaterialParameters, ShaderMaterial, Vector3
} from 'three'
...
export namespace HighPrecisionUtils {
   ...
   */
  export function createAttributes(positions: ArrayLike<number> | ArrayLike<Vector3>): {
   ...
   * @param positions - Array of positions.
   */
  export function convertPositions(positions: ArrayLike<number> | ArrayLike<Vector3>): {
    positions: number[]
  } {
    if (positions.length <= 0) {
      return { positions: [] }
