...
class ComputeExprDependencies implements ExprVisitor<void, ExprDependencies> {
  ...
  visitHasAttributeExpr(
    expr: HasAttributeExpr,
    context: ExprDependencies
  ): void {
    context.properties.add(expr.name)
  }
...
export abstract class LiteralExpr extends Expr {
  ...
  static fromValue(value: Value): Expr {
    switch (typeof value) {
      ...
      case 'object':
        return value === null
          ? NullLiteralExpr.instance
          : new ObjectLiteralExpr(value)
      default:
...
export class StringLiteralExpr extends LiteralExpr {
  ...
  get promotedValue(): RGBA | Pixels | undefined {
    if (this.m_promotedValue === undefined) {
      this.m_promotedValue =
        RGBA.parse(this.value) ?? Pixels.parse(this.value) ?? null
    }
...
export class CallExpr extends Expr {
  ...
  protected exprIsDynamic() {
    ...
    if (
      descriptor &&
      descriptor.isDynamicOperator &&
      descriptor.isDynamicOperator(this)
    ) {
      return true
    }
...
export class LookupExpr extends CallExpr {
  ...
  static parseArray(
    ...
    if (
      !Array.isArray(lookupTableNode) ||
      !(lookupTableExpr instanceof ObjectLiteralExpr)
    ) {
      throw new Error(
    ...
    const args = node
      .slice(2)
      .map((childExpr) => parseNode(childExpr, referenceResolverState))
...
export class CaseExpr extends Expr {
  ...
  protected exprIsDynamic() {
    return (
      this.branches.some(
        ([cond, branch]) => cond.isDynamic() || branch.isDynamic()
      ) || this.fallback.isDynamic()
    )
  }
}
...
function parseInterpolateExpr(
  ...
  const input =
    node[2] !== undefined
      ? parseNode(node[2], referenceResolverState)
      : undefined
...
function resolveReference(
  ...
  if (isInterpolatedPropertyDefinition(definitionValue)) {
    ...
    return Expr.fromJSON(
      interpolatedPropertyDefinitionToJsonExpr(definitionEntry)
    )
