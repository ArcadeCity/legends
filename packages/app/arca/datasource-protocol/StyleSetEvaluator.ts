...
import {
  BooleanLiteralExpr, CallExpr, CaseExpr, Env, Expr, ExprScope, ExprVisitor, HasAttributeExpr,
  InterpolateExpr, isJsonExpr, JsonExpr, LiteralExpr, LookupExpr, MatchExpr, NullLiteralExpr,
  NumberLiteralExpr, ObjectLiteralExpr, StepExpr, StringLiteralExpr, Value, VarExpr
} from './Expr'
import { ExprPool } from './ExprPool'
import {
  interpolatedPropertyDefinitionToJsonExpr, isInterpolatedPropertyDefinition
    ...
    if (call.op === 'all') {
      ...
      if (matched) {
        if (this._style.layer === undefined && matched.name === '$layer') {
          ...
          return undefined
        } else if (this._style._geometryType === undefined && matched.name === '$geometryType') {
...
class OptimizedSubSetKey {
  ...
  set(layer: string | undefined, geometryType: string | undefined, env?: Env): OptimizedSubSetKey {
 ...
 */
export type StyleSetOptions = Omit<DecoderOptions, 'languages'> & {
  styleSet: Styles
}
...
export class StyleSetEvaluator {
  ...
  wantsFeature(layer: string, geometryType?: string): boolean {
    return (
      this.getOptimizedStyleSet(this.m_tmpOptimizedSubSetKey.set(layer, geometryType)).length > 0
    )
  }
    ...
    if (style.when !== undefined) {
      try {
        ...
        if (isJsonExpr(style.minZoomLevel)) {
          style._minZoomLevelExpr = Expr.fromJSON(style.minZoomLevel, this.m_definitions).intern(
            this.m_exprPool
          )
        }

        if (isJsonExpr(style.maxZoomLevel)) {
          style._maxZoomLevelExpr = Expr.fromJSON(style.maxZoomLevel, this.m_definitions).intern(
            this.m_exprPool
          )
        }
      } catch (err) {
  ...
  private processStyle(env: Env, style: InternalStyle, result: Technique[]): boolean {
    ...
    if (this.m_layer !== undefined && style.layer !== undefined && style.layer !== this.m_layer) {
      return false
    }
    if (
    ...
    if (style.minZoomLevel !== undefined) {
      ...
      if (style._minZoomLevelExpr?.isDynamic() === false) {
        ...
        try {
          ...
          logger.error(
            `failed to evaluate expression '${JSON.stringify(style._minZoomLevelExpr)}': ${error}`
          )
        }
      }
    ...
    if (style.maxZoomLevel !== undefined) {
      ...
      if (style._maxZoomLevelExpr?.isDynamic() === false) {
        ...
        try {
          ...
          logger.error(
            `failed to evaluate expression '${JSON.stringify(style._maxZoomLevelExpr)}': ${error}`
          )
        }
      }
  ...
  private getDynamicTechniqueKey(style: InternalStyle, dynamicAttributes: Array<[string, Value]>) {
  ...
  private checkStyleDynamicAttributes(style: InternalStyle) {
    ...
    const processAttribute = (attrName: string, attrValue: Value | JsonExpr | undefined) => {
      ...
      if (isJsonExpr(attrValue)) {
        attrValue = Expr.fromJSON(attrValue, this.m_definitions, this.m_definitionExprCache).intern(
          this.m_exprPool
        )
      } else if (isInterpolatedPropertyDefinition(attrValue)) {
        // found a property using an object-like interpolation definition.
        attrValue = Expr.fromJSON(interpolatedPropertyDefinitionToJsonExpr(attrValue)).intern(
          this.m_exprPool
        )
      }
  ...
  private evaluateTechniqueProperties(style: InternalStyle, env: Env): Array<[string, Value]> {
    ...
    return style._dynamicTechniqueAttributes.map(([attrName, attrExpr]) => {
      try {
        const evaluatedValue = attrExpr.evaluate(env, ExprScope.Value, this.m_cachedResults)
  ...
  private createTechnique(style: InternalStyle, key: string, dynamicAttrs: Array<[string, Value]>) {
    const technique: any = {}
