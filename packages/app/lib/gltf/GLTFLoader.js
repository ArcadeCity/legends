...
class GLTFLoader extends Loader {
  ...
  load(url, onLoad, onProgress, onError) {
    ...
    // log('Here we have a fileloader but is it the right one?')
    ...
    log("About to use our FileLoader to load the file. Let's see what happens", url)

    loader.load(
      url,
      function (data) {
        try {
          log("Hey some fucking data. Let's see what it is", data.byteLength)
          scope.parse(
            data,
            resourcePath,
            function (gltf) {
      ...
      for (let i = 0; i < json.extensionsUsed.length; ++i) {
        ...
        switch (extensionName) {
          ...
          case EXTENSIONS.KHR_DRACO_MESH_COMPRESSION:
            log('Setting some D extension thing')
...
class GLTFDracoMeshCompressionExtension {
  constructor(json, dracoLoader) {
    log('GLTFDracoMeshCompressionExtension constructor!')
  ...
  decodePrimitive(primitive, parser) {
    log('Attempting to decode primitive with Draco extension...')
    ...
    return parser.getDependency('bufferView', bufferViewIndex).then(function (bufferView) {
      // log('Got bufferView dependency, now decoding with DracoLoader...??')
      return new Promise(function (resolve) {
        // log('Resolving promise attempting to decode...')
...
class GLTFParser {
  ...
  parse(onLoad, onError) {
    const parser = this
    // let origJson = this.json
    // origJson.textures = [{ extensions: null }]
    // const json = origJson
    // log('textures?', json.textures)
    const json = this.json
  ...
  getDependency(type, index) {
    ...
    if (!dependency) {
      ...
      // log('Dependency:', index, dependency)
  ...
  getDependencies(type) {
    ...
    if (!dependencies) {
      ...
      if (type === 'scene') {
        log(`LETS FUCKING GO. GETTING ${type} DEPENDENCIES`, defs)
      }
    ...
    function createDracoPrimitive(primitive) {
      return extensions[EXTENSIONS.KHR_DRACO_MESH_COMPRESSION]
        ...
        .then(function (geometry) {
          log('Hey returning geometry!', geometry)
